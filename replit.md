...
Lessons support **dual formats**:
- **Modules format** (new): Array of modules with internal timeline - used by the orchestrator
- **Timeline format** (legacy): Flat array of items - used by older lessons

Both formats are fully supported in the editor and renderer. Lessons are defined using a structured **JSON schema** supporting a variety of interactive elements such as AI tutor messages, images, quizzes, custom widgets, and reflection prompts. This content is stored in a **PostgreSQL database** with full CRUD API endpoints and an authoring interface in the admin panel.
...
- **Dual Format Support**: Generated lessons use the new `modules` format with rich, nested content structure.

## Recent Changes (2025-10-31)

### Fixed Issues
- **Editor Validation Bug**: The lesson editor now accepts both `modules` (new format from orchestrator) and `timeline` (legacy format)
- **Public View Bug**: Fixed issue where orchestrator-generated lessons appeared empty. The public view now correctly passes the `modules` field to the LessonRenderer
- **Content Display**: Lessons generated by the orchestrator now display their full content correctly (5,000-14,000 characters of interactive content)

### Current Status
- ✅ Orchestrator generates complete lessons with rich content
- ✅ Lessons are automatically persisted to the database
- ✅ "Super Prompt" button optimizes vague requests into detailed prompts
- ✅ Both `modules` and `timeline` formats are fully supported
- ⚠️ Generated lessons default to `published=false` and need manual publishing via the editor
...
- Lessons currently require authentication to view (under ProtectedRoute)
- Consider implementing automatic publishing option for orchestrator-generated lessons
- Consider making published lessons accessible without authentication